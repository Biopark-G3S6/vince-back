...
- [x] 1.1 Escrever, em `vince-docs`, o requisito funcional novo do grupo `ACS` — "Convidar usuário" —
...
- [x] 1.2 Acrescentar o requisito novo como origem de `INVITATION:CREATE`, `INVITATION:READ` e
...
- [x] 1.3 Acrescentar `INVITATION:CREATE`, `INVITATION:READ` e `INVITATION:REVOKE` à composição de
...
- [x] 1.4 Acrescentar à URS §2.4 o código de limite de usos atingido, com o requisito novo como
      origem. Verificação: o código consta do catálogo.
- [x] 1.5 Submeter as alterações de 1.1 a 1.4 à validação da parte interessada, junto das duas
...
- [x] 1.6 Atualizar o submódulo (`pnpm run docs:update`) e a carga inicial do catálogo, refletindo a
...
- [x] 2.1 Acrescentar a `src/modules/access/access.prisma` o model de convite: derivação do endereço
...
- [x] 2.2 Acrescentar o model da trilha de auditoria do convite, no schema do próprio módulo.
      Verificação: cenários "Emissão registrada", "Aceitação registrada" e "Senha ausente da trilha".
- [x] 2.3 Modelar o convite em `domain/`, com as invariantes das duas formas e as transições de estado
...
- [x] 2.4 Gerar o endereço por fonte criptograficamente segura, com no mínimo 128 bits, persistindo
      apenas sua derivação (decisão D3). Verificação: cenários "Endereço opaco" e "Valor não
      recuperável do armazenamento".
- [x] 2.5 Declarar a cadeia de não escalonamento em ponto único no `access` (decisão D5).
...
- [x] 3.1 Implementar, na fachada do `access`, a emissão de convite dirigido e de convite aberto, com
      ...
      ausente".
- [x] 3.2 Publicar, no módulo `institution`, a rota de emissão no escopo de uma instituição, validando
...
- [x] 3.3 Confirmar que `access` continua sem importar nada de `institution` (regra de módulo folha).
...
- [x] 4.1 Publicar, no `access`, a rota pública de consulta do convite pelo endereço, com resposta
...
- [x] 4.2 Publicar a rota pública de aceitação, criando a conta com papel e vínculo do convite e
      ...
      estabelecida".
- [x] 4.3 Implementar o consumo do convite por escrita condicional, na mesma transação da criação da
...
- [x] 4.4 Marcar as duas rotas como públicas na declaração da guarda de borda, que exige declaração
      explícita. Verificação: a aplicação sobe e as rotas respondem sem sessão.

## 5. Gestão dos convites emitidos

- [x] 5.1 Publicar a listagem paginada de convites por instituição, restrita a quem atua nela.
      Verificação: cenários "Listagem" e "Listagem não alcança outra instituição".
- [x] 5.2 Publicar a revogação, idempotente e sem efeito sobre contas já criadas. Verificação:
      cenários "Revogação", "Revogação repetida" e "Contas criadas permanecem".

## 6. Primeiro acesso do administrador inicial

- [x] 6.1 Fazer a carga inicial emitir e apresentar o meio de redefinição da conta inicial, apenas
...
- [x] 6.2 Documentar o primeiro acesso no `README.md` do repositório, no bloco de começando.
      Verificação: alguém que siga o README a partir de repositório limpo consegue entrar no sistema.

## 7. Encerramento

- [x] 7.1 Percorrer a cadeia inteira de ponta a ponta, como teste de jornada (`ADR-0024` §8): definir a
...
- [x] 7.2 Registrar a limitação de taxa das rotas públicas como pendência rastreável, junto da que
      `add-session-authentication` já abriu. Verificação: a pendência existe e nomeia as três rotas
      públicas do sistema.
- [x] 7.3 Atualizar os READMEs de `src/modules/access/` e `src/modules/institution/` com a repartição
      da validação entre emissor e mecanismo. Verificação: os dois arquivos a descrevem.
- [x] 7.4 Executar `pnpm run verify` inteiro. Verificação: tipos, lint, formatação e testes passam,
      com o Compose ativo.
