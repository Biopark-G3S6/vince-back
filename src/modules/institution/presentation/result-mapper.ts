 ...
 *
 * É **toda** a responsabilidade do controlador diante de uma falha: o caso de uso já
 * classificou com o código do catálogo, e o status HTTP de cada código está declarado em
 * ponto único. Não há decisão a tomar aqui — só a conversão.
 *
 * Código que a fachada devolva e o catálogo não conheça vira `INTERNAL_ERROR`, e não é
 * repassado ao cliente: um código fora do catálogo é defeito do servidor, e `ADR-0025`
 * §20 não admite que o cliente receba código que não possa traduzir.
 */
export function unwrap<T>(result: InstitutionResult<T>): T {
  if (result.ok) {
    return result.value;
  }
